...
wire [63:0] CTblock;
wire CTv,Tv,read;
wire [63:0] XsboxIN0, XsboxIN1, XsboxIN2, XsboxIN3,XsboxIN4;
wire [63:0] Xsbox0, Xsbox1, Xsbox2, Xsbox3, Xsbox4, p0,p1,p2;

integer inFile, outFile, outFile_h, i;
integer scan;
reg	[19:0] clk_count; // For Tracing/Debugginh 
reg [19:0] operation_count;

initial 
begin 
	clk	<= 1'b0; 
	clk_count <= 20'd0; 
	operation_count <= 20'd0;
end 
always 
begin 
	#1 clk <= ~clk;
...
//		$finish;
//	end
	outFile = $fopen("C:/My_Computer/Research/xilinx/ASCON_ecc/op_sequence.txt", "w");
	outFile_h = $fopen("C:/My_Computer/Research/xilinx/ASCON_ecc/op_sequence_h.txt", "w");
	// Please change your directory
end
...
//            $stop;
//        end
        $fwrite(outFile,"%b %b %b %b %b %b %b %b \n", XsboxIN0, XsboxIN1, XsboxIN2, XsboxIN3, XsboxIN4,p0,p1,p2);
        $fwrite(outFile_h,"%h %h %h %h %h %h %h %h \n", XsboxIN0, XsboxIN1, XsboxIN2, XsboxIN3, XsboxIN4,p0,p1,p2);
        if (Tv == 1'b1) begin
            operation_count <= operation_count+1;
        end
        if (operation_count == 2) begin $stop; end
    end
    $fclose(outFile);
end
endmodule
